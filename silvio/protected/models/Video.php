...
class Video extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('title, server, app, stream, template, http, rtmp', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, server, port, app, stream, template, http, rtmp', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('http',$this->http,true);
		$criteria->compare('rtmp',$this->rtmp,true);
